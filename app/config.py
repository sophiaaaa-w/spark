...
BASELINE_WINDOW_FALLBACK_DAYS = 180
BASELINE_MIN_SAMPLE = 5
BASELINE_EXCLUDE_RECENT_DAYS = 7      # 播放量未收敛
HISTORY_FETCH_MAX = 80                # 日期参数生效，一般用不到上限
AUTHOR_CACHE_TTL_DAYS = 7

# ---------------------------------------------------------------- 排序
#
# ── 排序 ────────────────────────────────────────────────────────
#
# 产品目标从「找出内容本身赢了的视频」改成「挖出品牌给达人的 brief，以及其中
# 哪部分真的有人看」。brief 体现在**趋同**上，趋同需要样本量才看得出来，
# 所以宁可牺牲排序精度换样本量。
#
# 因此砍掉了账号历史抓取和真实基线计算（省 $2.12/次 + 167 秒）。
# 实测 播放/粉丝 与真实基线倍数的秩相关 +0.780、Top20 重合 14/20，
# 作为免费代理够用了。baseline.py 保留在仓库里，以后做深度模式还能用。
#
...
STAGE_WEIGHTS = {
    "resolve": 2, "recall": 22, "filter": 1,
    "baseline_fetch": 28, "baseline_compute": 2,
    "download": 12, "frames": 8, "subtitles": 3,
    "cluster": 8, "patterns": 14,
}
